...
def construct_block(graph, num_block, ops):
    ...
    ops_name = ["sep 3x3", "sep 5x5", "avg 3x3", "max 3x3", "id", "add", "concat"]
...
def creat_graph(cell_arc):
    ...
    # Create h[i+1]
    G.add_node(999, label="h[i+1]", color="black", fillcolor="white", shape="box", style="filled")
    G.add_edge((len(cell_arc)+2)*10+3, 999)
...
def main():
    ...
    if(len(sys.argv) <= 1):
        ...
        for i in range(int(len(sys.argv)/2+1), len(sys.argv)):
