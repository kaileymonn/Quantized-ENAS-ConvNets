...
class MicroChild(Model):
                ...
                with tf.variable_scope("stack_{0}".format(conv_id)):
                    ...
                    with tf.variable_scope("bn"):
                        ...
                        offset = create_weight(
                            ...
                            initializer=zero_init, quantize=False)
                        scale = create_weight(
                            "scale", [num_possible_inputs, out_filters],  
                            initializer=one_init, quantize=False)
    ...
    def _build_train(self):
        ...
        #TODO Cast logits to fp32 since its fp16 now
        log_probs = tf.nn.sparse_softmax_cross_entropy_with_logits(
            logits=tf.cast(logits, tf.float32), labels=self.y_train)  

        self.loss = tf.reduce_mean(log_probs)  

        if self.use_aux_heads:
            log_probs = tf.nn.sparse_softmax_cross_entropy_with_logits(
                logits=tf.cast(self.aux_logits, tf.float32), labels=self.y_train)
