...
def get_train_ops(
    ...
    #TODO
    if not is_controller: 
        loss_scale_manager = tf.contrib.mixed_precision.FixedLossScaleManager(5000) # too big? try 10000
        loss_scale_optimizer = tf.contrib.mixed_precision.LossScaleOptimizer(opt, loss_scale_manager)
        grads_and_vars = loss_scale_optimizer.compute_gradients(loss, tf_variables)
    ...
    try:
        ...
        if not is_controller:
            ...
            grads_and_vars = [tuple([grads[i],tf_variables[i]]) for i,_ in enumerate(grads)]
            train_op = loss_scale_optimizer.apply_gradients(grads_and_vars, global_step=train_step)
        else:
